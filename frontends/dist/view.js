...
var _templateObject = _taggedTemplateLiteral(["<svg class='iconic $", "'><use xlink:href='#$", "' /></svg>"], ["<svg class='iconic $", "'><use xlink:href='#$", "' /></svg>"]),
    ...
    _templateObject11 = _taggedTemplateLiteral(["<div id='image' style='background-image: url($", ")'></div>"], ["<div id='image' style='background-image: url($", ")'></div>"]),
    _templateObject12 = _taggedTemplateLiteral(["<div id='image' style='background-image: url($", ");' class='full'></div>"], ["<div id='image' style='background-image: url($", ");' class='full'></div>"]),
    _templateObject13 = _taggedTemplateLiteral(["<div id='image' class='small' style='background-image: url($", "); width: $", "px; height: $", "px; margin-top: -$", "px; margin-left: -$", "px;'></div>"], ["<div id='image' class='small' style='background-image: url($", "); width: $", "px; height: $", "px; margin-top: -$", "px; margin-left: -$", "px;'></div>"]),
    _templateObject14 = _taggedTemplateLiteral(["\n\t        <h1>$", "</h1>\n\t        <div class='rows'>\n\t        "], ["\n\t        <h1>$", "</h1>\n\t        <div class='rows'>\n\t        "]),
    _templateObject15 = _taggedTemplateLiteral(["\n\t\t        <div class='row'>\n\t\t            <a class='name'>$", "</a>\n\t\t        "], ["\n\t\t        <div class='row'>\n\t\t            <a class='name'>$", "</a>\n\t\t        "]),
    _templateObject16 = _taggedTemplateLiteral(["<a class='tag'>$", "<span data-index='$", "'>", "</span></a>"], ["<a class='tag'>$", "<span data-index='$", "'>", "</span></a>"]),
    _templateObject17 = _taggedTemplateLiteral(["$", "", ""], ["$", "", ""]),
    _templateObject18 = _taggedTemplateLiteral(["<span class='attr_$", "'>$", "</span>"], ["<span class='attr_$", "'>$", "</span>"]),
...
build.album = function (data) {
	...
	var thumbPath = _lychee$retinize.path;
	var thumbRetina = _lychee$retinize.hasRetina;

	html += lychee.html(_templateObject5, data.id, data.thumbs[2], data.thumbs[1], thumbPath, thumbRetina, data.title, data.title, data.sysdate);
...
build.photo = function (data) {
	...
	var thumbPath = _lychee$retinize2.path;
	var thumbRetina = _lychee$retinize2.hasRetina;

	html += lychee.html(_templateObject7, data.album, data.id, thumbPath, data.title, data.title);
...
build.imageview = function (data, size, visibleControls) {
	...
	if (size === 'big') {
		...
		if (visibleControls === true) html += lychee.html(_templateObject11, data.url);else html += lychee.html(_templateObject12, data.url);
	} else if (size === 'medium') {

		if (visibleControls === true) html += lychee.html(_templateObject11, data.medium);else html += lychee.html(_templateObject12, data.medium);
	} else if (size === 'small') {

		if (visibleControls === true) html += lychee.html(_templateObject13, data.url, data.width, data.height, parseInt(data.height / 2 - 20), data.width / 2);else html += lychee.html(_templateObject13, data.url, data.width, data.height, parseInt(data.height / 2), data.width / 2);
	}
...
build.uploadModal = function (title, files) {
	...
	while (i < files.length) {
		...
		html += lychee.html(_templateObject15, file.name);
...
build.tags = function (tags) {
	...
	if (tags !== '') {
		...
		tags.forEach(function (tag, index, array) {
			html += lychee.html(_templateObject16, tag, index, build.iconic('x'));
		});
	} else {
...
api.post = function (fn, params, callback) {
	...
	fnMap = {
		'Album::getAll': {
			...
			parse: function parse(data) {
				...
				return { albums: data, num: data.length, "smartalbums": { "unsorted": { "thumbs": [], "num": 0 }, "public": { "thumbs": [], "num": 0 }, "starred": { "thumbs": [], "num": 0 }, "recent": { "thumbs": [], "num": 0 } } };
			}
		},
		...
		'Album::get': {
			url: function url(params) {
				return '/albums/' + params.albumID + '/photos';
			},
			parse: function parse(data) {
				ret = { id: "8", title: "Noebl 2015 [A]", "public": "1", "description": "", "visible": "1", "downloadable": "0", "sysdate": "January 2016", "password": "1", "thumbs": [], 'content': {} };
				for (o in data) {
					ret.content[data[o].id] = { album: "8",
						cameraDate: "1",
						id: data[o].id,
						nextPhoto: "14528873776057",
						previousPhoto: "14528873469070",
						"public": "0",
						star: "0",
						sysdate: "15 January 2016",
						tags: "",
						thumbUrl: data[o].url_info['base'] + data[o].url_info['extension'],
						title: "Noël 2015 5",
						url: data[o].url_info['base'] + data[o].url_info['extension'] };
				}
				return ret;
			}
		}
	...
	var success = function success(data) {
		...
		setTimeout(function () {
			return loadingBar.hide();
		}, 100);

		// Catch errors
		if (data.error) {
			api.onError(data.error, params, data);
			return false;
		}
	...
	$.ajax({
		type: 'GET',
		url: api.path + fnMap[fn].url(params),
		data: params,
		dataType: 'json',
		success: success,
		error: error
...
header.setTitle = function () {
	...
	var $title = header.dom('.header__title'),
	    html = lychee.html(_templateObject17, title, build.iconic('caret-bottom'));
...
sidebar.createStructure.album = function (data) {
	...
	structure.share = {
		title: 'Share',
		type: sidebar.types.DEFAULT,
		rows: [{ title: 'Public', value: _public }, { title: 'Visible', value: visible }, { title: 'Downloadable', value: downloadable }, { title: 'Password', value: password }]
	};
...
sidebar.render = function (structure) {
	...
	var renderDefault = function renderDefault(section) {
		...
		section.rows.forEach(function (row) {
			...
			value = lychee.html(_templateObject18, row.title.toLowerCase(), value);

			// Add edit-icon to the value when editable
			if (row.editable === true) value += ' ' + build.editIcon('edit_' + row.title.toLowerCase());

			_html += lychee.html(_templateObject19, row.title, value);
		});
	...
	var renderTags = function renderTags(section) {
		...
		_html += lychee.html(_templateObject20, section.title, section.title.toLowerCase(), section.value, editable);
...
var loadingBar = { show: function show() {}, hide: function hide() {} },
    imageview = $('#imageview');
...
var getPhotoSize = function getPhotoSize(photo) {
